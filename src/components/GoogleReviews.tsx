      ...
      {[1,2,3,4,5,6].map(i => (
        <div key={i} className="bg-white rounded-3xl border border-stone-100 p-6 animate-pulse">
          ...
          <div className="flex items-center gap-3">
            ...
            <div><div className="h-3 bg-stone-200 rounded w-24 mb-1"/><div className="h-2.5 bg-stone-200 rounded w-16"/></div>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-center gap-3 mb-10">
        ...
        <a href={`https://www.google.com/maps/search/${encodeURIComponent(data.name)}`}
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {data.reviews.map((r, i) => (
          <div key={i}
            className="bg-white border border-stone-100 rounded-3xl p-6 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all">
            {/* Stars */}
            <div className="flex gap-1 mb-4">
              {[1,2,3,4,5].map(s => (
                <Star key={s} className="w-4 h-4"
                  style={{ fill: s <= r.rating ? "#F5B829" : "transparent",
                           color: s <= r.rating ? "#F5B829" : "#D1D5DB" }} />
              ))}
            </div>
            {/* Text */}
