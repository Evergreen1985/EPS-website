...
export default function PhotoUploader({ sectionId, sectionName, uploadedBy, uploadedByRole, children = [], onUploaded }: Props) {
  ...
  const upload = async () => {
    ...
    for (let i = 0; i < previews.length; i++) {
      ...
      try {
        ...
        const res  = await fetch("/api/photos/upload", {
          ...
          body: JSON.stringify({ photoUrl, sectionId, sectionName, title: p.title || null, uploadedBy, uploadedByRole, isFeatured: p.featured }),
        });
      ...
      } catch (e: any) { setError(e?.message || "Upload failed"); setUploading(false); return; }
    }

    setProgress(100); setUploading(false); setCurrent("");
    ...
    if (uploaded.length > 0) {
      ...
      setResults(aiResults); setAiTagging(false);
    }
  ...
  return (
    <div style={{ fontFamily:"'Quicksand',sans-serif" }}>
      ...
      <div onClick={() => fileRef.current?.click()}
        ...
        <div style={{ fontSize:"11px", color:"#6B7A99" }}>JPG, PNG, WEBP · Multiple files · Any size</div>
      ...
      {previews.length > 0 && (
        <div style={{ marginTop:"14px" }}>
          <div style={{ fontSize:"12px", fontWeight:700, color:"#6B7A99", marginBottom:"8px" }}>{previews.length} photo{previews.length>1?"s":""} ready</div>
          ...
          <div style={{ display:"flex", gap:"8px", alignItems:"center", flexWrap:"wrap" }}>
            ...
            <button onClick={() => { setPreviews([]); setError(""); }}
              style={{ background:"#EDE8DF", color:"#6B7A99", border:"none", borderRadius:"12px", padding:"9px 14px", fontSize:"12px", cursor:"pointer" }}>Clear</button>
            <div style={{ fontSize:"11px", color:"#6B7A99", display:"flex", alignItems:"center", gap:"4px" }}>
              <Sparkles style={{ width:"12px", height:"12px", color:"#F5B829" }} /> AI auto-tags after upload
            </div>
          </div>
            ...
            <div key={i} style={{ display:"flex", gap:"10px", marginBottom:"8px", alignItems:"flex-start" }}>
              ...
              <div>
                ...
                <div style={{ display:"flex", gap:"4px", flexWrap:"wrap", marginTop:"3px" }}>
                  {r.tags.map(tag => <span key={tag} style={{ fontSize:"10px", background:"#FAF0E8", borderRadius:"20px", padding:"2px 8px", color:"#6B7A99" }}>#{tag}</span>)}
                </div>
              </div>
            </div>
          ))}
